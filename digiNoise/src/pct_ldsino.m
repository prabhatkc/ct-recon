function [lsino,yi] = pct_ldsino(bi, hsino, k, sig, hdose)
...
% % for high dose use lower than 0.85 factor for poisson as 
% poisson(test, [],'factor', 0.3)
% yi = poisson(yb, [], 'try_poissrnd', true) + sqrt(sig)*randn(size(yb)); 

if strcmp(hdose, "T")
    yi = poisson(yb, [],'factor', 0.3) + sqrt(sig)*randn(size(yb));
    %yi = poisson(yb, [], 'try_poissrnd', true) + sqrt(sig)*randn(size(yb));
else
